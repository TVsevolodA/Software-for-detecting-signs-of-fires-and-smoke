...
module.exports= class Report{
    constructor(db, report_data) {
        this.pool = db;
        this.report_id = Number(report_data.report_id);
        this.event_based = report_data.event_based;
        this.number_incident = Number(report_data.number_incident);
        this.datetime = report_data.datetime;
        this.duty = Number(report_data.duty);
    ...
    async update() {
        ...
        await this.pool.query(`UPDATE notifications SET report_compiled = TRUE WHERE incident_id = $1`,
        [this.number_incident]);
    }
}